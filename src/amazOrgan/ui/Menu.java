...
//	}
	...
	public static void main(String[] ars) {
		...
		/*System.out.println("Adding a receptor into the data base: ");
		Integer dni = 11;
		String date = "2000-03-25";
		...
		Boolean alive = true;
		Integer urgency = 2;
		Antibody antibody = new Antibody (1, false, true);
		Antigen antigen = new Antigen (1, true, false, false, false, true, false);
		Float lat = 3.45f;
		Float lon = 45.5f;
		Location location = new Location (3, lat, lon);
		Type_organ type_organ = new Type_organ(2, "feet", 45);
		Request request = new Request(3, type_organ, lon, true, null);
		Receptor r = new Receptor (dni, dob, status, bloodtype, alive, urgency, antigen, antibody, location, request);
		  			  
		System.out.println(r);
		receptorManager.addReceptor(r); 
		Doctor d = new Doctor(4, 617144309,"Eustaquio");
		
		doctorManager.addDoctor(d);*/
		
		Receptor r = receptorManager.getReceptor(11);
		
		
		
		Doctor d = doctorManager.getDoctor(4);
		
		receptorManager.listMyReceptors(1);
		
		
		
//		Receptor r = receptorManager.getReceptor(1);
//		System.out.println(r);
